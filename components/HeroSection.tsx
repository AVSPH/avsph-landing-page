...
export default function HeroSection() {
  ...
  const column3 = [...staffImages.sort(() => Math.random() - 0.5), ...staffImages, ...staffImages]; // Randomize slightly
  ...
  const column7 = [...staffImages.sort(() => Math.random() - 0.5), ...staffImages, ...staffImages];
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20 bg-slate-50">
      ...
      <div className="absolute inset-0 overflow-hidden z-0 mask-gradient-vertical">
        <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-7 gap-4 h-[120vh] -mt-[10vh] opacity-100">
          ...
          <MarqueeColumn images={column2} direction="down" duration={220} className="mt-[-50px]" />

          {/* Column 3 (Visible on all) */}
          <MarqueeColumn images={column3} direction="up" duration={210} />

          {/* Column 4 (Hidden on mobile < md) */}
          <MarqueeColumn images={column4} direction="down" duration={230} className="hidden md:block mt-[-30px]" />

          {/* Column 5 (Hidden on mobile < md) */}
          <MarqueeColumn images={column5} direction="up" duration={215} className="hidden md:block" />

          {/* Column 6 (Hidden on tablet < lg) */}
          <MarqueeColumn images={column6} direction="down" duration={250} className="hidden lg:block mt-[-40px]" />

          {/* Column 7 (Hidden on tablet < lg) */}
          <MarqueeColumn images={column7} direction="up" duration={215} className="hidden lg:block" />
      ...
      <div className="container relative z-10 text-center px-4">
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          >
            Hire Once. <span className="text-primary  ">Scale Fast.</span> Cancel Anytime.
          </motion.h1>
          ...
          >
            <Link href="#contact" className="btn-primary rounded-full px-8 py-4 text-lg shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all w-full sm:w-auto">
              Book a Free Call
            </Link>
            <Link href="#services" className="px-8 py-4 bg-white/80 backdrop-blur-sm rounded-full font-semibold border-2 border-slate-200 text-slate-700 hover:border-blue-600 hover:text-blue-600 transition-all flex items-center justify-center gap-2 w-full sm:w-auto shadow-sm">
              Explore Services
            </Link>
          </motion.div>
...
function MarqueeColumn({ images, direction = "up", duration = 40, className = "" }: { images: string[], direction?: "up" | "down", duration?: number, className?: string }) {
  return (
